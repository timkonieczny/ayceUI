 ...
 * @constructor
 */
Ayce.LODObject3D = function (geometriesHighToLowRes, distance, camera) {
    ...
    var activeLevel = 0;
    var cameraPosition = camera.getGlobalPosition();
    ...
    this.update = function(){
        ...
        if(!areVectorsEqual(lastCameraPosition, cameraPosition) || !areVectorsEqual(lastObjectPosition, scope.position)) {  // Only calculate LOD if camera or object changed
            ...
            if (!isLODAssigned) { // if camera is closer than first LOD
                setLevelActive(0);
            }
