...
//Scene Management
    ...
    this.updateScene = function () {
        if(recalcBuffers){
            ...
            calcO3DBuffers(pickingBufferObjects);
            recalcBuffers = false;
        }
        ...
        for(i=0; i < pickingBufferObjects.length; i++){
            pickingBufferObjects[i].update();
        }
        renderer.update(camera, objects, transparentObjects, pickingBufferObjects);
    ...
    this.drawScene = function () {
        if(this.render && ! recalcBuffers) {
            ...
            //render scene
            renderer.render(camera, objects, transparentObjects, pickingBufferObjects);
        }
    };
    ...
    this.addToScene = function (object, enablePicking) {
        ...
        //Add O3D to Scene
            ...
            if(enablePicking) { // TODO: don't use randoms for color values
                object.pickingColor = [Math.random(), Math.random(), Math.random(), 1.0];
                ...
                for (var i = 0; i < copy.vertices.length / 3; i++) {
                    copy.colors.push(copy.pickingColor[0], copy.pickingColor[1], copy.pickingColor[2], copy.pickingColor[3]);
                }
                ...
                pickingBufferObjects.push(copy);
            }
        }
    ...
    this.removeFromScene = function (object) {
        ...
        //Remove o3D from Scene
            ...
            for (i=0; i < pickingBufferObjects.length; i++) {
                if (pickingBufferObjects[i] === object) {
                    pickingBufferObjects.splice(i, 1);
                }
            }
        }
        else if(object instanceof Ayce.Sound){
